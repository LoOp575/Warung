...
import { Barang, OmzetHarian, formatRupiah, getStatus, hitungPembagian } from "@/lib/store";
    ...
    <div className="p-4 lg:p-8">
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4 mb-6">
        <div className="bg-white rounded-2xl shadow-sm border border-gray-100 p-4 card-hover">
          ...
          <p className="text-lg lg:text-xl font-bold text-navy-900">{formatRupiah(omzet)}</p>
        </div>
        <div className="bg-white rounded-2xl shadow-sm border border-gray-100 p-4 card-hover">
          <p className="text-xs text-gray-500 mb-1">Restock (70%)</p>
          <p className="text-lg lg:text-xl font-bold text-primary-600">{formatRupiah(pembagian.restock)}</p>
        </div>
        <div className="bg-white rounded-2xl shadow-sm border border-gray-100 p-4 card-hover">
          <p className="text-xs text-gray-500 mb-1">Tabungan (15%)</p>
          <p className="text-lg lg:text-xl font-bold text-green-600">{formatRupiah(pembagian.tabungan)}</p>
        </div>
        <div className="bg-white rounded-2xl shadow-sm border border-gray-100 p-4 card-hover">
          <p className="text-xs text-gray-500 mb-1">Growth (10%)</p>
          <p className="text-lg lg:text-xl font-bold text-purple-600">{formatRupiah(pembagian.growth)}</p>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-4 mb-6">
        <div className="bg-white rounded-2xl shadow-sm border border-gray-100 p-4 card-hover">
          ...
          <p className="text-xl font-bold text-orange-600">{formatRupiah(pembagian.kas)}</p>
        </div>
        <div className="bg-white rounded-2xl shadow-sm border border-gray-100 p-4 card-hover">
        ...
        <div className="space-y-3">
          {alerts.length === 0 ? (
            <div className="flex items-center gap-3 p-3 bg-green-50 rounded-xl">
              <div className="w-3 h-3 bg-green-500 rounded-full"></div>
              <p className="text-sm text-green-700">Semua stok aman! Tidak ada yang perlu dibeli.</p>
            </div>
          ) : (
            alerts.map((b) => (
