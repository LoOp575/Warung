...
export default function StokPage({
  ...
  const handleSimpanBarang = () => {
    ...
    onTambahBarang({
      ...
      satuan: addSatuan.trim() || "pcs",
      stok: parseInt(addStok) || 0,
      stokNormal: parseInt(addStokNormal) || 10,
      minimum: parseInt(addMinimum) || 3,
    });
    setShowTambah(false);
    setAddNama("");
    setAddKategori("rokok");
    setAddSatuan("");
    setAddStok("");
    setAddStokNormal("");
    setAddMinimum("");
    ...
    <div className="p-4 lg:p-8">
      ...
      <div className="flex flex-wrap gap-3 mb-6">
        <select
          ...
          className="bg-white border border-gray-200 rounded-xl px-4 py-2 text-sm focus:ring-2 focus:ring-primary-400 outline-none"
        >
          ...
          filtered.map((b) => {
            ...
            return (
              <div
                ...
                onClick={() => {
                  setEditBarang(b);
                  setEditStokVal(String(b.stok));
                }}
                className="bg-white rounded-2xl shadow-sm border border-gray-100 p-5 card-hover cursor-pointer"
              >
                <div className="flex justify-between items-start mb-3">
                  ...
                  <span
                    className={`text-xs font-bold px-2 py-1 rounded-lg ${statusClass}`}
                  >
                    {s.status}
                  </span>
                </div>
                <div className="mb-3">
                  ...
                  <div className="w-full bg-gray-100 rounded-full h-2 overflow-hidden">
                    <div
                      className={`${barColor} h-2 rounded-full progress-bar`}
                      style={{ width: `${persen}%` }}
                    ></div>
                  </div>
                </div>
            ...
            <div className="space-y-4">
              <div>
                ...
                <input
                  ...
                  className="w-full px-4 py-3 border border-gray-200 rounded-xl focus:ring-2 focus:ring-primary-400 outline-none"
                  placeholder="Contoh: Pandemas"
                />
              </div>
              <div>
                <label className="text-sm text-gray-600 mb-1 block">
              ...
              <div>
                ...
                <input
                  ...
                  className="w-full px-4 py-3 border border-gray-200 rounded-xl focus:ring-2 focus:ring-primary-400 outline-none"
                  placeholder="Contoh: slop, renceng, pcs"
                />
              </div>
              <div>
                <label className="text-sm text-gray-600 mb-1 block">
                ...
                <input
                  type="number"
                  value={addStok}
                  onChange={(e) => setAddStok(e.target.value)}
                  className="w-full px-4 py-3 border border-gray-200 rounded-xl focus:ring-2 focus:ring-primary-400 outline-none"
                  placeholder="0"
                />
              </div>
              <div>
                ...
                <input
                  type="number"
                  value={addStokNormal}
                  onChange={(e) => setAddStokNormal(e.target.value)}
                  className="w-full px-4 py-3 border border-gray-200 rounded-xl focus:ring-2 focus:ring-primary-400 outline-none"
                  placeholder="10"
                />
              </div>
        ...
        <div className="fixed inset-0 bg-black/50 z-[100] flex items-center justify-center p-4">
          <div className="bg-white rounded-2xl w-full max-w-sm p-6">
            ...
            <p className="text-sm text-gray-500 mb-3">
              {editBarang.nama} ({editBarang.stok} {editBarang.satuan})
            </p>
            <input
              type="number"
              value={editStokVal}
              onChange={(e) => setEditStokVal(e.target.value)}
              className="w-full px-4 py-3 border border-gray-200 rounded-xl focus:ring-2 focus:ring-primary-400 outline-none mb-4"
            ...
            <div className="flex gap-3">
              ...
              <button
                onClick={handleHapus}
                className="bg-red-50 text-red-600 px-4 py-3 rounded-xl font-medium hover:bg-red-100 transition-colors"
              >
                Hapus
              </button>
            </div>
