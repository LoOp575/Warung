...
import { ModalData, formatRupiah, getTanggalIndo } from "@/lib/store";
      ...
      <div className="bg-white rounded-2xl shadow-sm border border-gray-100 p-6 mb-6 card-hover">
        ...
        <div className="grid grid-cols-2 gap-4 mb-6">
          <div>
            ...
            <p className="text-lg font-bold text-navy-900">{formatRupiah(modalData.awal)}</p>
          </div>
          <div>
            <p className="text-xs text-gray-500">Target</p>
            <p className="text-lg font-bold text-primary-600">{formatRupiah(modalData.target)}</p>
          </div>
        </div>
        ...
        <div className="flex gap-3">
          ...
          <button onClick={handleTambah} className="bg-green-500 text-white px-5 py-3 rounded-xl font-medium hover:bg-green-600 transition-colors">
            + Tambah
          </button>
        </div>
      </div>
